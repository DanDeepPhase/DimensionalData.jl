using DimensionalData, Test, Dates, Statistics
using DimensionalData.LookupArrays, Rasters.Dimensions
using DimensionalData: upsample, downsample
using DimensionalData: aggregate, disaggregate, Every
...
array1 = DimArray(data1, dimz)
array2 = DimArray(data2, dimz)
array1a = DimArray(data3, dimz)
array2a = DimArray(data4, dimz)
stack1 = DimStack(array1, array2; keys=(:array1, :array2))
stack2 = DimStack(array1a, array2a; keys=(:array1, :array2))
...
@testset "Aggregate with a function and Every" begin
    @test aggregate(sum, array1, 3) == [72 99]
    @test aggregate(median, array1, 3) == [8 11]
    @test aggregate(sum, array1, (3, 2)) == [45 57 69]
    A = aggregate(sum, array1, (3, 2))
